...
func renderWebSocketTS(baseURL string, registry *tsInterfaceRegistry, metas []wsFuncMeta) (string, error) {
	...
	b.WriteString("    this.serialize = options?.serialize ?? ((value: TSend) => normalizeRequestJSON(value));\n")
	b.WriteString("    this.deserialize = options?.deserialize ?? ((value: unknown) => normalizeResponseJSON(value) as TReceive);\n")
	b.WriteString("\n")
