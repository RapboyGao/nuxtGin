...
func renderWebSocketTS(baseURL string, registry *tsInterfaceRegistry, metas []wsFuncMeta) (string, error) {
	...
	b.WriteString("}\n\n")
		...
		b.WriteString("    const runtimeConfigPublic =\n")
		b.WriteString("      (globalThis as any).useRuntimeConfig?.()?.public ??\n")
		b.WriteString("      (window as any)?.__NUXT__?.config?.public;\n")
		b.WriteString("    const ginPort = runtimeConfigPublic?.ginPort;\n")
