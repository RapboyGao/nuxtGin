...
func renderWebSocketTS(baseURL string, registry *tsInterfaceRegistry, metas []wsFuncMeta) (string, error) {
	...
	b.WriteString("  private readonly typedHandlers = new Map<number, { type: string; listener: (message: TReceive) => void }>();\n")
	b.WriteString("  private typedHandlerSeq = 0;\n\n")
	...
	b.WriteString("  }\n\n")
	b.WriteString("  addTypeHandler(type: string, handler: (message: TReceive) => void, options?: TypeHandlerOptions<TReceive>): number {\n")
	...
	b.WriteString("    const handlerID = ++this.typedHandlerSeq;\n")
	b.WriteString("    this.typedHandlers.set(handlerID, { type, listener: wrapped });\n")
	b.WriteString("    return handlerID;\n")
	b.WriteString("  }\n\n")
	b.WriteString("  removeTypeHandler(handlerID: number): boolean {\n")
	b.WriteString("    const handler = this.typedHandlers.get(handlerID);\n")
	b.WriteString("    if (!handler) return false;\n")
	b.WriteString("    const listeners = this.typedListeners.get(handler.type);\n")
	b.WriteString("    if (listeners) {\n")
	b.WriteString("      listeners.delete(handler.listener);\n")
	b.WriteString("      if (listeners.size === 0) this.typedListeners.delete(handler.type);\n")
	b.WriteString("    }\n")
	b.WriteString("    this.typedHandlers.delete(handlerID);\n")
	b.WriteString("    return true;\n")
	b.WriteString("  }\n\n")
	b.WriteString("  clearTypeHandlers(type?: string): void {\n")
	b.WriteString("    if (type === undefined) {\n")
	b.WriteString("      this.typedListeners.clear();\n")
	b.WriteString("      this.typedHandlers.clear();\n")
	b.WriteString("      return;\n")
	b.WriteString("    }\n")
	b.WriteString("    this.typedListeners.delete(type);\n")
	b.WriteString("    for (const [id, handler] of this.typedHandlers.entries()) {\n")
	b.WriteString("      if (handler.type === type) this.typedHandlers.delete(id);\n")
	b.WriteString("    }\n")
	b.WriteString("  }\n\n")
	b.WriteString("  onType(type: string, handler: (message: TReceive) => void, options?: TypeHandlerOptions<TReceive>): () => void {\n")
	b.WriteString("    const handlerID = this.addTypeHandler(type, handler, options);\n")
	b.WriteString("    return () => {\n")
	b.WriteString("      this.removeTypeHandler(handlerID);\n")
	b.WriteString("    };\n")
	b.WriteString("  }\n\n")
	b.WriteString("  addTypedHandler<TPayload>(\n")
	...
	b.WriteString("    return this.addTypeHandler(type, (message) => {\n")
	...
	b.WriteString("    options?: TypedHandlerOptions<TReceive, TPayload>\n")
	b.WriteString("  ): () => void {\n")
	b.WriteString("    const handlerID = this.addTypedHandler(type, handler, options);\n")
	b.WriteString("    return () => {\n")
	b.WriteString("      this.removeTypeHandler(handlerID);\n")
	b.WriteString("    };\n")
	b.WriteString("  }\n\n")
