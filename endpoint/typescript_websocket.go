...
func renderWebSocketTS(baseURL string, registry *tsInterfaceRegistry, metas []wsFuncMeta) (string, error) {
	...
	b.WriteString("}\n\n")
		...
		b.WriteString("    const nuxtIsDevelopment = (window as any)?.__NUXT__?.config?.public?.isDevelopment;\n")
		b.WriteString("    if (typeof nuxtIsDevelopment === 'boolean') return nuxtIsDevelopment;\n")
		b.WriteString("  }\n")
		...
		b.WriteString("  if (typeof window !== 'undefined') {\n")
		b.WriteString("    const ginPort = (window as any)?.__NUXT__?.config?.public?.ginPort;\n")
